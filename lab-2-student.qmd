...
dim(surveys)
```
There are 30463 observations (rows) and 15 variables (colums)
...
The variables are record ID, month, day, year, plot ID, species ID, sex, hindfoot length, weight, date, day of the week, plot type, genus, species, and taxa.
...
ggplot(data = surveys,
  ...
  labs(
    ...
    y="", 
    title= "Relationship Between Rodent Weight and Hindfoot Length",
    subtitle= "Y-axis representing Hindfoot Length (cm)")
```
...
ggplot(data = surveys,
  ...
  geom_jitter(alpha=0.5, color="green4")+
  geom_boxplot()+
  labs(
    x="Species", 
    y="", 
...
Help from: https://stackoverflow.com/questions/1330989/rotating-and-spacing-axis-labels-in-ggplot2
...
There is a significant difference between at least one species weight and another
